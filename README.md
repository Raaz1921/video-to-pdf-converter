# 📹 Video to PDF Converter (Browser-Based)

A **browser-based Video to PDF Converter** that converts videos into **screenshot-based PDFs** using **fixed time intervals** or **smart scene detection** — fully **client-side**, with **no backend**, **no uploads**, and **no server cost**.

> ✅ Runs entirely in your browser  
> ✅ Works offline after loading  
> ❌ No video is uploaded anywhere  

---

## 🔍 What is Video to PDF?

**Video to PDF** means extracting visual frames (screenshots) from a video and compiling them into a multi-page PDF document.

Each page represents a meaningful moment in the video (slides, scenes, key frames).

This project does exactly that — **locally in your browser**.

---

## 🚀 Features

- 📁 Upload local video files (MP4, WebM, etc.)
- 🖼️ Convert **video frames to PDF pages**
- ⏱️ Fixed interval capture (every N seconds)
- 🎬 Smart scene detection using visual difference
- 🔒 Max-gap protection to avoid long gaps
- 🧾 Multi-page PDF generation
- ✏️ **User-defined PDF file name**
- 📊 Live progress bar
- 🧠 Preview screenshots before PDF creation
- 🌙 Dark mode UI
- 🔐 100% client-side (no uploads, no tracking)

---

## 🛠️ Tech Stack

- **HTML5**
- **CSS3**
- **Vanilla JavaScript**
- **jsPDF**
- **Canvas API**
- **HTML5 Video API**

No frameworks.  
No backend.  
No database.

---

## 📂 Project Structure

```text
/
├── index.html   (or video-to-pdf-2.0.html)
└── README.md